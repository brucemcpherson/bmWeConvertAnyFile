...
const Compress = (() => {
  ...
  const decompress = (str) => {
    return JSON.parse(
      Utilities.unzip(Utilities.newBlob(Utilities.base64Decode(str), 'application/zip'))[0].getDataAsString()
    )
  }
   ...
   */
  const compress = (obj) =>
    Utilities.base64Encode(Utilities.zip([Utilities.newBlob(stringify(obj))]).getBytes())
